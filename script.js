...
*/
function play() {
 const answer = randomNumber(1,100);
 let guesses = 0;
 let guess = 0;
 
 do {
    guess = prompt('Giskaðu á tölu milli 1 - 100');
    if(guess === null) {
        break;
    }
    guess = parseGuess(guess);
    alert(getResponse(guess, answer));
    guesses++;
    if(guess === answer)
    {
        games.push(guesses);
...
function calculateAverage(){
    ...
    return Math.round(100*total/games.length)/100;
}
...
function getResponse(guess, correct){
    ...
    let distance = Math.abs(guess - correct);
    if(distance === 0)
    {
        return "Rétt";
    }
